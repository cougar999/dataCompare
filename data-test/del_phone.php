...
$query = $reader->query("SELECT * FROM `data_new` WHERE mark= 1 AND difference LIKE '%s' LIMIT 0, 1000", array('%[["home_phone"%'));
...
while ($row = $query->each()) {
	if ($row && $row['difference']) {
		...
		foreach ($text as $key => $value) {
			...
			if ($len < 3 && $value[0] == 'home_phone' && $value[1] == 'mobile_phone') {
				# change mark's value
				//$datawrite = $writer->query("UPDATE `__data_new` SET `mark`=5 WHERE `email`='%s'", array($row['email']))->debug();
				echo "yes1";

			} elseif ($len < 2 && $value[0] == 'home_phone') {
				//$datawrite = $writer->query("UPDATE `__data_new` SET `mark`=5 WHERE `email`='%s'", array($row['email']))->debug();
				echo "yes2";
			} else {
				print_r('no <br>');
