...
$result2 = array();
$query = $reader->query("SELECT * FROM tocompare WHERE 1=1 LIMIT 0, 10000", array());
...
while ($row = $query->each()) {
	...
	if (count($result) > 0) {
		...
		if ($result2[$key] && $result2[$key]['ID']) {
			#new record
			echo "new record!";
			$datawrite = $writer->query("UPDATE `__tocompare` SET `mark`='new', `difference`='new record' WHERE `email`='%s'", array($key));
		
		} else {
			#mark 
			$text = json_encode($result2[$key]);
			echo "find a difference at least";
			$datawrite = $writer->query("UPDATE `__tocompare` SET `mark`= 1, `difference`='%s' WHERE `email`='%s'", array($text, $key));			
